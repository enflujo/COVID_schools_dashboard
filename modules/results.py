...
def save(args, hvec, soln_ind, number_nodes, nodes):
    ...
    if school_levels[0] is True:
        nodes_leves['preschool_nodes'] = [str(n) for n in nodes['preschool'][0]]
        nodes_res['pre_inf_list'] = list()
    if school_levels[1] is True:
        nodes_leves['primary_nodes'] = [str(n) for n in nodes['primary'][0]]
        nodes_res['pri_inf_list'] = list()
    if school_levels[0] is True:
        nodes_leves['highschool_nodes'] = [str(n) for n in nodes['highschool'][0]]
        nodes_res['high_inf_list'] = list()
    nodes_leves['work_nodes'] = [str(n) for n in nodes['work'][0]]
    nodes_res['work_inf_list'] = list()
    nodes_leves['other_nodes'] = [str(n) for n in nodes['other'][0]]
    nodes_res['comm_inf_list'] = list()
    ...
    for i in range(args.number_trials):
        ...
        pre_inf = df_soln_ind_i[nodes_leves['preschool_nodes']] == 2
        pre_inf = pre_inf.any()
        pre_inf = pre_inf[pre_inf == True].shape[0] # n infected in sim
        nodes_res['pre_inf_list'].append(pre_inf/nodes['preschool'][1])

        pri_inf = df_soln_ind_i[nodes_leves['primary_nodes']] == 2
        pri_inf = pri_inf.any()
        pri_inf = pri_inf[pri_inf == True].shape[0] # n infected in sim
        nodes_res['pri_inf_list'].append(pri_inf/nodes['primary'][1])

        high_inf = df_soln_ind_i[nodes_leves['highschool_nodes']] == 2
        high_inf = high_inf.any()
        high_inf = high_inf[high_inf == True].shape[0] # n infected in sim
        nodes_res['high_inf_list'].append(high_inf/nodes['highschool'][1])

        work_inf = df_soln_ind_i[nodes_leves['work_nodes']] == 2
        work_inf = work_inf.any()
        work_inf = work_inf[work_inf == True].shape[0] # n infected in sim
        nodes_res['work_inf_list'].append(work_inf/nodes['work'][1])

        comm_inf = df_soln_ind_i[nodes_leves['other_nodes']] == 2
        comm_inf = comm_inf.any()
        comm_inf = comm_inf[comm_inf == True].shape[0] # n infected in sim
        nodes_res['comm_inf_list'].append(comm_inf/nodes['other'][1])

    out = {'preschool': sum(nodes_res['pre_inf_list'])/len(nodes_res['pre_inf_list']),
           'primary': sum(nodes_res['pri_inf_list'])/len(nodes_res['pri_inf_list']),
           'highschool': sum(nodes_res['high_inf_list'])/len(nodes_res['high_inf_list']),
           'work': sum(nodes_res['work_inf_list'])/len(nodes_res['work_inf_list']),
           'comm': sum(nodes_res['comm_inf_list'])/len(nodes_res['comm_inf_list'])
           }
