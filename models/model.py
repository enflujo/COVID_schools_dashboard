...
def plot_single_cumulative(
    cumulative_history, tvec, n, ymax=1, scale=1, int=0, Tint=0, plotThis=False, plotName="test"
):
    """
...
def get_peaks_iter(soln, tvec, int=0, Tint=0, loCI=5, upCI=95):
    ...
    # Final values
    print(
        "Final recovered: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(soln[:, -1, 6]),
            100 * np.percentile(soln[:, -1, 6], loCI),
            100 * np.percentile(soln[:, -1, 6], upCI),
        )
    )
    print(
        "Final deaths: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(soln[:, -1, 5]),
            100 * np.percentile(soln[:, -1, 5], loCI),
            100 * np.percentile(soln[:, -1, 5], upCI),
        )
    )
    print(
        "Remaining infections: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(all_cases[:, -1]),
            100 * np.percentile(all_cases[:, -1], loCI),
            100 * np.percentile(all_cases[:, -1], upCI),
        )
    )
    ...
    print(
        "Peak I1: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(peaks), 100 * np.percentile(peaks, loCI), 100 * np.percentile(peaks, upCI)
        )
    )
    peaks = np.amax(soln[:, :, 3], axis=1)
    print(
        "Peak I2: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(peaks), 100 * np.percentile(peaks, loCI), 100 * np.percentile(peaks, upCI)
        )
    )
    peaks = np.amax(soln[:, :, 4], axis=1)
    print(
        "Peak I3: {:4.2f}% [{:4.2f}, {:4.2f}]".format(
            100 * np.average(peaks), 100 * np.percentile(peaks, loCI), 100 * np.percentile(peaks, upCI)
        )
    )
    ...
    print(
        "Time of peak I1: avg {:4.2f} days, median {:4.2f} days [{:4.2f}, {:4.2f}]".format(
            np.average(tpeak), np.median(tpeak), np.percentile(tpeak, loCI), np.percentile(tpeak, upCI)
        )
    )
    tpeak = np.argmax(soln[:, :, 3], axis=1) * delta_t - time_int
    print(
        "Time of peak I2: avg {:4.2f} days, median {:4.2f} days [{:4.2f}, {:4.2f}]".format(
            np.average(tpeak), np.median(tpeak), np.percentile(tpeak, loCI), np.percentile(tpeak, upCI)
        )
    )
    tpeak = np.argmax(soln[:, :, 4], axis=1) * delta_t - time_int
    print(
        "Time of peak I3: avg {:4.2f} days, median {:4.2f} days [{:4.2f}, {:4.2f}]".format(
            np.average(tpeak), np.median(tpeak), np.percentile(tpeak, loCI), np.percentile(tpeak, upCI)
        )
    )
    ...
    time_all_extinct = {
        "days": np.average(time_all_extinct_arr),
        "int1": np.percentile(time_all_extinct_arr, loCI),
        "int2": np.percentile(time_all_extinct_arr, upCI),
    }

    print(
        "Time of extinction of all infections post intervention: {:4.2f} days  [{:4.2f}, {:4.2f}]".format(
            np.average(time_all_extinct_arr),
            np.percentile(time_all_extinct_arr, loCI),
            np.percentile(time_all_extinct_arr, upCI),
        )
    )
...
def get_peaks_single_daily(daily_incidence, int=0, Tint=0):
    ...
    # Peak incidence
    print("Peak daily I1: {:3.1f}%".format(100 * np.max(daily_incidence[:, 2])))
    print("Peak daily I2: {:3.1f}%".format(100 * np.max(daily_incidence[:, 3])))
    print("Peak daily I3: {:3.1f}%".format(100 * np.max(daily_incidence[:, 4])))
    print("Peak daily D: {:3.1f}%".format(100 * np.max(daily_incidence[:, 5])))

    # Time of peak incidence
    print("Time of peak daily I1: {:3.1f} days".format(np.argmax(daily_incidence[:, 2]) + 1 - time_int))
    print("Time of peak daily I2: {:3.1f} days".format(np.argmax(daily_incidence[:, 3]) + 1 - time_int))
    print("Time of peak daily I3: {:3.1f} days".format(np.argmax(daily_incidence[:, 4]) + 1 - time_int))
    print("Time of peak daily D: {:3.1f} days".format(np.argmax(daily_incidence[:, 5]) + 1 - time_int))
...
def plot_iter_cumulative_shade(
    soln_cum, tvec, n, ymax=1, scale=1, int=0, Tint=0, loCI=5, upCI=95, plotThis=False, plotName="test"
):
...
def plot_iter_daily_shade(
    soln_inc, n, ymax=1, scale=1, int=0, Tint=1, loCI=5, upCI=95, plotThis=False, plotName="test"
):
