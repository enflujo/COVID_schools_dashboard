...
for i, inter_ in tqdm(enumerate(intervention_effcs), total=len(intervention_effcs)):
    ...
    if inter_ < 1.0:
        ...
        res_mean = res_read.groupby("tvec").mean()
        res_mean = res_mean.reset_index()
        res_loCI = res_read.groupby("tvec").quantile(0.05)
        res_loCI = res_loCI.reset_index()
        res_upCI = res_read.groupby("tvec").quantile(0.95)
        res_upCI = res_upCI.reset_index()
        res_tvec = list(res_mean["tvec"])
        res_inc = model.get_daily_iter(res_read, res_tvec)
        res_mean_inc = res_inc.groupby("tvec").mean()
        res_mean_inc = res_mean_inc.reset_index()
        res_loCI_inc = res_inc.groupby("tvec").quantile(0.05)
        res_loCI_inc = res_loCI_inc.reset_index()
        res_upCI_inc = res_inc.groupby("tvec").quantile(0.95)
        res_upCI_inc = res_upCI_inc.reset_index()
    elif inter_ == 1.0:
        res_read = load_results_dyn(
            "soln_cum",
            os.path.join(
                "results",
                "no_intervention",
    ...
    plt.gca().set_prop_cycle(None)
    plt.fill_between(
        res_mean_inc["tvec"], res_loCI_inc["E"] * pop, res_upCI_inc["E"] * pop, color=interv_color_label[i], alpha=0.3
    )
    # plt.plot([20,20],[0,100],'k--',alpha=0.2)
...
plt.savefig(
    os.path.join(
        figures_path,
        "{}_dailyIncidence_ComNumber_schoolcap_{}_n_{}.png".format(args.type_sim, args.school_occupation, str(pop)),
    ),
    dpi=400,
    transparent=True,
    bbox_inches="tight",
