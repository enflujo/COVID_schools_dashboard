...
figures_path = "".join([base, config_data.loc["figures_dir"][1]])
results_path = "".join([base, config_data.loc["results_dir"][1]])
ages_data_path = "".join([base, config_data.loc["bogota_age_data_dir"][1]])
houses_data_path = "".join([base, config_data.loc["bogota_houses_data_dir"][1]])
...
def load_results_ints(type_res, n, int_effec, schl_occup, path=results_path):
    read_path = os.path.join(
        path,
        "{}_inter_{}_schoolcap_{}_mask_N95_peopleMasked_1.0_ventilation_3_ID_ND_{}.csv".format(
            str(n), str(int_effec), str(schl_occup), type_res
        ),
    )
    read_file = pd.read_csv(read_path)
    return read_file
...
for c, cap_ in tqdm(enumerate(school_caps), total=len(school_caps)):
    ...
    save_path = os.path.join(
        figures_path,
        "cases_evolution",
        "{}_lin_{}_dynamics_schoolcap_{}_n_{}.png".format(plot_state, args.type_sim, cap_, str(pop)),
    )
...
for c, cap_ in tqdm(enumerate(school_caps), total=len(school_caps)):
    ...
    for i, inter_ in enumerate(intervention_effcs):
        ...
        plt.gca().set_prop_cycle(None)
        plt.fill_between(
            res_median["tvec"], res_loCI["E"] * 100, res_upCI["E"] * 100, color=interv_color_label[i], alpha=0.3
        )
        plt.axvspan(0, 20, color="gray", alpha=0.05)
        plt.annotate("Schools \n closed", (0, 1.2), size=9)
        plt.annotate("Schools \n open", (22, 1.2), size=9)
    ...
    save_path = os.path.join(
        figures_path,
        "cases_evolution",
        "{}_log_{}_dynamics_schoolcap_{}_n_{}.png".format(plot_state, args.type_sim, cap_, str(pop)),
    )
...
for c, cap_ in tqdm(enumerate(school_caps), total=len(school_caps)):
    ...
    for i, inter_ in enumerate(intervention_effcs):
        ...
        if inter_ < 1.0:
            ...
            res_median_inc = res_inc.groupby("tvec").median()
            res_median_inc = res_median_inc.reset_index()
            res_loCI_inc = res_inc.groupby("tvec").quantile(alpha / 2)
            res_loCI_inc = res_loCI_inc.reset_index()
            res_upCI_inc = res_inc.groupby("tvec").quantile(1 - alpha / 2)
            res_upCI_inc = res_upCI_inc.reset_index()
        # read results with no intervention
        elif inter_ == 1.0:
            res_read = load_results_dyn("soln", os.path.join("results", "no_intervention", str(pop)))
            res_median = res_read.groupby("tvec").median()
        ...
        plt.fill_between(
            res_median_inc["tvec"],
            res_loCI_inc[plot_state] * pop,
            res_upCI_inc[plot_state] * pop,
            color=interv_color_label[i],
            alpha=0.3,
        )
