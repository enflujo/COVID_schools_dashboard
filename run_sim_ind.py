...
parser.add_argument("--hh_size", default=4, type=int, help="Speficy the number of individials in a household")
...
parser.add_argument("--fraction_people_masks", default=1.0, type=float, help="Fraction value of people wearing masks")
parser.add_argument(
    "--masks_type",
    default="N95",
    type=str,
...
parser.add_argument("--n_teachers", default=5, type=float, help="Number of teachers at school")
parser.add_argument("--n_teachers_vacc", default=5, type=float, help="Number of vaccinated teachers at school")
parser.add_argument("--n_school_going", default=50, type=float, help="Number of students at school")
parser.add_argument("--n_classrooms", default=10, type=float, help="Number of classrooms at school")
parser.add_argument("--classroom_size", default=15, type=float, help="Number of students per classroom")

parser.add_argument("--height_room", default=3.1, type=float, help="Schools height of classroom")
parser.add_argument("--school_length_room", default=10.0, type=float, help="school length of classroom")
parser.add_argument("--school_width_room", default=10.0, type=float, help="school length of classroom")
...
parser.add_argument("--school_mean", default=9.4, type=float, help="school degree distribution (mean)")
parser.add_argument("--school_std", default=1.8, type=float, help="school degree distribution (standard deviation)")
parser.add_argument("--school_size", default=35, type=float, help="Number of students per classroom")
parser.add_argument("--school_r", default=1, type=float, help="Correlation in school layer")
...
dist_of_pop = [school_pop / total_pop_BOG, work, very_young + university + elderly]
...
matrix_household = create_networks.create_fully_connected(
    [args.hh_size], age_tracker_all, np2.arange(0, pop, 1), df_run_params, args.delta_t
)

## School
matrix_school = create_networks.create_external_corr_schools(
    school_going,
    school_degree,
    n_classrooms,
    r_school,
    ...
    df_run_params,
    args.delta_t,
    args.school_length_room,
    args.school_width_room,
    args.height_room,
...
matrix_work = create_networks.create_external_corr(
    working, work_degree, n_work, r_work, work_indx, job_place, age_tracker, df_run_params, args.delta_t
)
...
df_results_soln.to_csv(
    path_save + "/{}_schoolcap_{}_soln.csv".format(str(number_nodes), str(args.school_occupation)), index=False
)
df_results_soln_cum.to_csv(
    path_save + "/{}_schoolcap_{}_soln_cum.csv".format(str(number_nodes), str(args.school_occupation)), index=False
)
