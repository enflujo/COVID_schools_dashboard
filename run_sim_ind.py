...
parser.add_argument('--hh_size', default=4, type=int,
                    help='Speficy the number of individials in a household')
...
parser.add_argument('--ventilation_level', default=3, type=float,
...
parser.add_argument('--n_teachers', default=5, type=float,
                    help='Number of teachers at school')
parser.add_argument('--n_teachers_vacc', default=5, type=float,
                    help='Number of vaccinated teachers at school')
parser.add_argument('--n_school_going', default=50, type=float,
                    help='Number of students at school')
parser.add_argument('--n_classrooms', default=10, type=float,
                    help='Number of classrooms at school')
parser.add_argument('--classroom_size', default=15, type=float,
...
parser.add_argument('--school_length_room', default=10.0, type=float,
                    help='school length of classroom')
parser.add_argument('--school_width_room', default=10.0, type=float,
                    help='school length of classroom')
...
parser.add_argument('--school_mean', default=9.4, type=float,
                    help='school degree distribution (mean)')
parser.add_argument('--school_std', default=1.8, type=float,
                    help='school degree distribution (standard deviation)')
parser.add_argument('--school_size', default=35, type=float,
                    help='Number of students per classroom')
parser.add_argument('--school_r', default=1, type=float,
                    help='Correlation in school layer')
...
'''
Define how does households are going to be interpreted
'''
...
school_pop_ = [args.n_school_going] + [args.n_teachers]
school_pop = sum(school_pop_)

work_pop_teachers = sum(work_)

# Frac of population that is school going, working, preschool or elderly
dist_of_pop = [school_pop/total_pop_BOG,
               work,
...
classify_pop = np2.random.choice(['school','work','other'], size=pop, p=dist_of_pop)
...
school_going = args.n_school_going
N_schools_s = 1
...
### School -------------------------------------------------------
mean, std = args.school_mean, args.school_std
p = 1-(std**2/mean)
n_binom = mean/p
school_degree = np2.random.binomial(n_binom, p, size = school_going)
n_classrooms = args.n_classrooms
r_school = args.school_r

school_clroom = np2.random.choice(np.arange(0,n_classrooms+1,1),size=school_going)

# Assign ages to the school going population acc. to their proportion from the census data
prob = []
school_pop = sum(school_pop_)

for i in range(0,len(school_pop_)):
    prob.append(school_pop_[i]/school_pop)
age_group_school = np2.random.choice(np.array([1,7]),size=school_going,p=prob,replace=True)
...
## Households
matrix_household = create_networks.create_fully_connected([args.hh_size],age_tracker_all,np2.arange(0,pop,1),df_run_params,args.delta_t)

## School
matrix_school = create_networks.create_external_corr_schools(pop,school_going,school_degree,n_classrooms,r_school,school_indx,school_clroom,age_tracker,df_run_params,args.delta_t
    ,args.school_length_room,args.school_width_room,args.height_room,args.ventilation_out,inhalation_mask,exhalation_mask,args.fraction_people_masks,args.duration_event)
...
df_results_soln.to_csv(path_save+'/{}_schoolcap_{}_soln.csv'.format(str(number_nodes),str(args.school_occupation)), index=False)
df_results_soln_cum.to_csv(path_save+'/{}_schoolcap_{}_soln_cum.csv'.format(str(number_nodes),str(args.school_occupation)), index=False)
